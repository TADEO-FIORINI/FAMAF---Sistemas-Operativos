...
/********************* DIRECTORY FUNCTIONS *********************/
void fat_file_dentry_remove_child(fat_file parent, fat_file child) {
    child->dentry->base_name[0] = 0xe5;
    write_dir_entry(parent, child->dentry, child->pos_in_parent);
    if (errno != 0) {
        return;
    }
    DEBUG("Removing child \"%s\" in \"%s\" in position %u", child->name,
          parent->filepath, child->pos_in_parent);
}
    ...
    for (disk_dentry_ptr = (fat_dir_entry)buffer; disk_dentry_ptr <= end_ptr;
        ...
        if (ignore_dentry(disk_dentry_ptr)) {
            if (disk_dentry_ptr->base_name[0] == 0xe5 &&
                (disk_dentry_ptr->attribs & (FILE_ATTRIBUTE_SYSTEM))) {
                // 102 es el valor de 'f'
                disk_dentry_ptr->base_name[0] = 102;
                fat_dir_entry new_entry =
                    init_direntry_from_buff(disk_dentry_ptr);
                fat_file is_fs_log = init_file_from_dentry(new_entry, dir);
                // Por las dudas dejamos como estaba lo anterior
                disk_dentry_ptr->base_name[0] = 0xe5;
                // Volvemos a esconderlo
                is_fs_log->dentry->base_name[0] = 0xe5;
                // Si no es fs.log lo destruimos
                if (strcmp(is_fs_log->name, "fs.log")) {
                    fat_file_destroy(is_fs_log);
                } else {
                    // Si son iguales lo agregamos a GList
                    (*elems) = g_list_append((*elems), is_fs_log);
                }
            }
            continue;
        }
...
ssize_t fat_file_pread(fat_file file, void *buf, size_t size, off_t offset,
    ...
    DEBUG("file: %s\n size: %lu\n offset: %lu", file->name, size, offset);
...
ssize_t fat_file_pwrite(fat_file file, const void *buf, size_t size,
    ...
    // If next cluster of current cluster is EOC then we assign it a new one
    next_cluster = fat_table_get_next_cluster(file->table, cluster);
    if (fat_table_is_EOC(file->table, next_cluster)) {
        // Le asignamos un nuevo cluster
        next_cluster = fat_table_get_next_free_cluster(file->table);
        fat_table_set_next_cluster(file->table, cluster, next_cluster);
        fat_table_set_next_cluster(file->table, next_cluster,
                                   FAT_CLUSTER_END_OF_CHAIN);
    }
    ...
    while (bytes_remaining > 0) {
        ...
        if (bytes_remaining > 0) {
            next_cluster = fat_table_get_next_cluster(file->table, cluster);
            if (fat_table_is_EOC(file->table, next_cluster)) {
                // We assing a new cluster if is at EOC
                next_cluster = fat_table_get_next_free_cluster(file->table);
                fat_table_set_next_cluster(file->table, cluster, next_cluster);
            } else {
